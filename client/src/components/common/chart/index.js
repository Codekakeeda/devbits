...
const CommonChart = ({ type = 'candlestick', api_response }) => {
    ...
    useEffect(() => {
        ...
        for (const timestamp in api_response.items) {
            if (api_response.items.hasOwnProperty(timestamp)) {
                const item = api_response.items[timestamp];

                // Convert the date and time stamp to a JavaScript Date object
                const date = new Date(item.date_utc * 1000);

                // Create a data point object in the desired format
                const dataPoint = {
                    x: date,
                    y: [item.open, item.high, item.low, item.close],
                };

                // Add the data point object to the converted data array
                convertedData.push(dataPoint);
            }
        }

        // Create a series object using the converted data array
        const newSeries = [
            {
                data: convertedData,
            },
    ...
    const options = {
        ...
        prices: series //.find((week) => week.name === "2023-W03-05").data
        //  [
        //     {
        //         data: [{
    ...
    return (
        <div className="candlestick-chart">
            <Chart options={options} series={options.prices} type={type} height={350} />
        </div>
    );
};
